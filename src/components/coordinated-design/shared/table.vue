<!-- an editble, searchable and configurable table -->
...
export default {
    ...
    methods: {
            ...
            cloneData.map((dataItem) => {
                Object.keys(dataItem).map((key) => {
                    if (typeof dataItem[key] === 'object') {
                        dataItem[key] = JSON.stringify(dataItem[key]);
                    };
                });
            });
            let res = [];
        ...
        setFilter () {
            if (this.filterField && this.filter) {
                this.thisTableData = this.thisTableData.filter(
                    item => item[this.filterField].indexOf(this.filter) > -1
                );
            }
        },
    },
