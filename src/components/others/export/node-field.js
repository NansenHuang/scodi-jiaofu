const NodeField = {
    'Section': [
        {
            'label': 'project',
            'value': 'project',
            'enable': false
        },
        {
            'label': 'id',
            'value': 'id',
            'enable': true
        },
        {
            'label': 'name',
            'value': 'name',
            'enable': true
        },
        {
            'label': 'section',
            'value': 'sectionID',
            'enable': true
        },
        {
            'label': 'alignmentID',
            'value': 'rightPart.alignmentID',
            'enable': true
        },
        {
            'label': 'alignmentCnName',
            'value': 'rightPart.alignmentCnName',
            'enable': true
        },
        {
            'label': 'alignmentStationMark',
            'value': 'rightPart.stationMark',
            'enable': true
        },
        {
            'label': 'alignmentStartStation',
            'value': 'rightPart.startStation',
            'enable': true
        },
        {
            'label': 'alignmentEndStation',
            'value': 'rightPart.endStation',
            'enable': true
        },
        {
            'label': 'alignmentStartStationChain',
            'value': 'rightPart.startStationChain',
            'enable': true
        },
        {
            'label': 'alignmentEndStationChain',
            'value': 'rightPart.endStationChain',
            'enable': true
        },
        {
            'label': 'tableType',
            'value': 'tableType',
            'enable': true
        }
    ],
    'Alignment': [
        {
            'label': 'project',
            'value': 'project',
            'enable': false
        },
        {
            'label': 'id',
            'value': 'id',
            'enable': true
        },
        {
            'label': 'owerType',
            'value': 'owerType',
            'enable': true
        },
        {
            'label': 'owerID',
            'value': 'owerID',
            'enable': true
        },
        {
            'label': 'owerCnName',
            'value': 'owerCnName',
            'enable': true
        },
        {
            'label': 'direction',
            'value': 'direction',
            'enable': true
        },
        {
            'label': 'alignmentEnName',
            'value': 'alignmentEnName',
            'enable': true
        },
        {
            'label': 'alignmentCnName',
            'value': 'alignmentCnName',
            'enable': true
        },
        {
            'label': 'stationMark',
            'value': 'stationMark',
            'enable': true
        },
        {
            'label': 'startStation',
            'value': 'startStation',
            'enable': true
        },
        {
            'label': 'endStation',
            'value': 'endStation',
            'enable': true
        },
        {
            'label': 'tableType',
            'value': 'tableType',
            'enable': true
        }
    ],
    'Blignment': [
        {
            'label': 'project',
            'value': 'project',
            'enable': false
        },
        {
            'label': 'id',
            'value': 'id',
            'enable': true
        },
        {
            'label': 'alignmentID',
            'value': 'alignmentID',
            'enable': true
        },
        {
            'label': 'alignmentCnName',
            'value': 'alignmentCnName',
            'enable': true
        },
        {
            'label': 'region',
            'value': 'region',
            'enable': true
        },
        {
            'label': 'stationMark',
            'value': 'stationMark',
            'enable': true
        },
        {
            'label': 'startStation',
            'value': 'startStation',
            'enable': true
        },
        {
            'label': 'endStation',
            'value': 'endStation',
            'enable': true
        },
        {
            'label': 'alignmentFile',
            'value': 'alignmentFiles',
            'enable': true
        },
        {
            'label': 'tableType',
            'value': 'tableType',
            'enable': true
        }
    ],
    'Clignment': [
        {
            'label': 'project',
            'value': 'project',
            'enable': false
        },
        {
            'label': 'id',
            'value': 'id',
            'enable': true
        },
        {
            'label': 'alignmentID',
            'value': 'alignmentID',
            'enable': true
        },
        {
            'label': 'alignmentCnName',
            'value': 'alignmentCnName',
            'enable': true
        },
        {
            'label': 'direction',
            'value': 'direction',
            'enable': true
        },
        {
            'label': 'region',
            'value': 'region',
            'enable': true
        },
        {
            'label': 'stationMark',
            'value': 'stationMark',
            'enable': true
        },
        {
            'label': 'startStation',
            'value': 'startStation',
            'enable': true
        },
        {
            'label': 'endStation',
            'value': 'endStation',
            'enable': true
        },
        {
            'label': 'leftParts',
            'value': 'leftPartsCopy',
            'enable': true
        },
        {
            'label': 'rightParts',
            'value': 'rightPartsCopy',
            'enable': true
        },
        {
            'label': 'corridorFiles',
            'value': 'corridorFiles',
            'enable': true
        },
        {
            'label': 'tableType',
            'value': 'tableType',
            'enable': true
        }
    ],
    'Chain': [
        {
            'label': 'project',
            'value': 'project',
            'enable': false
        },
        {
            'label': 'id',
            'value': 'id',
            'enable': true
        },
        {
            'label': 'alignmentID',
            'value': 'alignmentID',
            'enable': true
        },
        {
            'label': 'alignmentCnName',
            'value': 'alignmentCnName',
            'enable': true
        },
        {
            'label': 'measureStation',
            'value': 'config.measureStation',
            'enable': true
        },
        {
            'label': 'brokenChainStation',
            'value': 'config.station',
            'enable': true
        },
        {
            'label': 'type',
            'value': 'config.type',
            'enable': true
        },
        {
            'label': 'length',
            'value': 'config.length',
            'enable': true
        },
        {
            'label': 'sumLong',
            'value': 'config.sumLong',
            'enable': true
        },
        {
            'label': 'sumShort',
            'value': 'config.sumShort',
            'enable': true
        },
        {
            'label': 'continuousStation',
            'value': 'config.sequentStation',
            'enable': true
        },
        {
            'label': 'stationMark',
            'value': 'stationMark',
            'enable': true
        },
        {
            'label': 'tableType',
            'value': 'tableType',
            'enable': true
        }
    ],
    'Interchange': [
        {
            'label': 'project',
            'value': 'project',
            'enable': false
        },
        {
            'label': 'id',
            'value': 'id',
            'enable': true
        },
        {
            'label': 'interchangeCnName',
            'value': 'interchangeCnName',
            'enable': true
        },
        {
            'label': 'interchangeEnName',
            'value': 'interchangeEnName',
            'enable': true
        },
        {
            'label': 'section',
            'value': 'sectionID',
            'enable': true
        },
        {
            'label': 'leftAlignmentID',
            'value': 'leftParts.alignmentID',
            'enable': true
        },
        {
            'label': 'leftAlignmentCnName',
            'value': 'leftParts.alignmentCnName',
            'enable': true
        },
        {
            'label': 'leftStationMark',
            'value': 'leftParts.stationMark',
            'enable': true
        },
        {
            'label': 'leftStartStation',
            'value': 'leftParts.startStation',
            'enable': true
        },
        {
            'label': 'leftEndStation',
            'value': 'leftParts.endStation',
            'enable': true
        },
        {
            'label': 'leftStartStationChain',
            'value': 'leftParts.startStationChain',
            'enable': true
        },
        {
            'label': 'leftEndStationChain',
            'value': 'leftParts.endStationChain',
            'enable': true
        },
        {
            'label': 'rightAlignmentID',
            'value': 'rightPart.alignmentID',
            'enable': true
        },
        {
            'label': 'rightAlignmentCnName',
            'value': 'rightPart.alignmentCnName',
            'enable': true
        },
        {
            'label': 'rightStationMark',
            'value': 'rightPart.stationMark',
            'enable': true
        },
        {
            'label': 'rightStartStation',
            'value': 'rightPart.startStation',
            'enable': true
        },
        {
            'label': 'rightEndStation',
            'value': 'rightPart.endStation',
            'enable': true
        },
        {
            'label': 'rightStartStationChain',
            'value': 'rightPart.startStationChain',
            'enable': true
        },
        {
            'label': 'rightEndStationChain',
            'value': 'rightPart.endStationChain',
            'enable': true
        },
        {
            'label': 'desc',
            'value': 'desc',
            'enable': true
        },
        {
            'label': 'tableType',
            'value': 'tableType',
            'enable': true
        }
    ],
    'Servicearea': [
        {
            'label': 'project',
            'value': 'project',
            'enable': false
        },
        {
            'label': 'id',
            'value': 'id',
            'enable': true
        },
        {
            'label': 'serviceAreaCnName',
            'value': 'serviceareaCnName',
            'enable': true
        },
        {
            'label': 'serviceAreaEnName',
            'value': 'serviceareaEnName',
            'enable': true
        },
        {
            'label': 'section',
            'value': 'sectionID',
            'enable': true
        },
        {
            'label': 'leftAlignmentID',
            'value': 'leftParts.alignmentID',
            'enable': true
        },
        {
            'label': 'leftAlignmentCnName',
            'value': 'leftParts.alignmentCnName',
            'enable': true
        },
        {
            'label': 'leftStationMark',
            'value': 'leftParts.stationMark',
            'enable': true
        },
        {
            'label': 'leftStartStation',
            'value': 'leftParts.startStation',
            'enable': true
        },
        {
            'label': 'leftEndStation',
            'value': 'leftParts.endStation',
            'enable': true
        },
        {
            'label': 'leftStartStationChain',
            'value': 'leftParts.startStationChain',
            'enable': true
        },
        {
            'label': 'leftEndStationChain',
            'value': 'leftParts.endStationChain',
            'enable': true
        },
        {
            'label': 'rightAlignmentID',
            'value': 'rightPart.alignmentID',
            'enable': true
        },
        {
            'label': 'rightAlignmentCnName',
            'value': 'rightPart.alignmentCnName',
            'enable': true
        },
        {
            'label': 'rightStationMark',
            'value': 'rightPart.stationMark',
            'enable': true
        },
        {
            'label': 'rightStartStation',
            'value': 'rightPart.startStation',
            'enable': true
        },
        {
            'label': 'rightEndStation',
            'value': 'rightPart.endStation',
            'enable': true
        },
        {
            'label': 'rightStartStationChain',
            'value': 'rightPart.startStationChain',
            'enable': true
        },
        {
            'label': 'rightEndStationChain',
            'value': 'rightPart.endStationChain',
            'enable': true
        },
        {
            'label': 'desc',
            'value': 'desc',
            'enable': true
        },
        {
            'label': 'tableType',
            'value': 'tableType',
            'enable': true
        }
    ],
    'Parkinglot': [
        {
            'label': 'project',
            'value': 'project',
            'enable': false
        },
        {
            'label': 'id',
            'value': 'id',
            'enable': true
        },
        {
            'label': 'parkingAreaCnName',
            'value': 'stopCnName',
            'enable': true
        },
        {
            'label': 'parkingAreaEnName',
            'value': 'stopEnName',
            'enable': true
        },
        {
            'label': 'section',
            'value': 'sectionID',
            'enable': true
        },
        {
            'label': 'leftAlignmentID',
            'value': 'leftParts.alignmentID',
            'enable': true
        },
        {
            'label': 'leftAlignmentCnName',
            'value': 'leftParts.alignmentCnName',
            'enable': true
        },
        {
            'label': 'leftStationMark',
            'value': 'leftParts.stationMark',
            'enable': true
        },
        {
            'label': 'leftStartStation',
            'value': 'leftParts.startStation',
            'enable': true
        },
        {
            'label': 'leftEndStation',
            'value': 'leftParts.endStation',
            'enable': true
        },
        {
            'label': 'leftStartStationChain',
            'value': 'leftParts.startStationChain',
            'enable': true
        },
        {
            'label': 'leftEndStationChain',
            'value': 'leftParts.endStationChain',
            'enable': true
        },
        {
            'label': 'rightAlignmentID',
            'value': 'rightPart.alignmentID',
            'enable': true
        },
        {
            'label': 'rightAlignmentCnName',
            'value': 'rightPart.alignmentCnName',
            'enable': true
        },
        {
            'label': 'rightStationMark',
            'value': 'rightPart.stationMark',
            'enable': true
        },
        {
            'label': 'rightStartStation',
            'value': 'rightPart.startStation',
            'enable': true
        },
        {
            'label': 'rightEndStation',
            'value': 'rightPart.endStation',
            'enable': true
        },
        {
            'label': 'rightStartStationChain',
            'value': 'rightPart.startStationChain',
            'enable': true
        },
        {
            'label': 'rightEndStationChain',
            'value': 'rightPart.endStationChain',
            'enable': true
        },
        {
            'label': 'desc',
            'value': 'desc',
            'enable': true
        },
        {
            'label': 'tableType',
            'value': 'tableType',
            'enable': true
        }
    ],
    'Culvert': [
        {
            'label': 'project',
            'value': 'project',
            'enable': false
        },
        {
            'label': 'id',
            'value': 'id',
            'enable': true
        },
        {
            'label': 'section',
            'value': 'sectionID',
            'enable': true
        },
        {
            'label': 'alignmentID',
            'value': 'alignmentID',
            'enable': true
        },
        {
            'label': 'alignmentCnName',
            'value': 'alignmentCnName',
            'enable': true
        },
        {
            'label': 'stationMark',
            'value': 'stationMark',
            'enable': true
        },
        {
            'label': 'station',
            'value': 'station',
            'enable': true
        },
        {
            'label': 'stationChain',
            'value': 'stationChain',
            'enable': true
        },
        {
            'label': 'leftParts',
            'value': 'leftPartsCopy',
            'enable': true
        },
        {
            'label': 'rightParts',
            'value': 'rightPartsCopy',
            'enable': true
        },
        {
            'label': 'desc',
            'value': 'desc',
            'enable': true
        },
        {
            'label': 'bimFile',
            'value': 'bimFiles',
            'enable': true
        },
        {
            'label': 'angle',
            'value': 'angle',
            'enable': true
        },
        {
            'label': 'direction',
            'value': 'direction',
            'enable': true
        },
        {
            'label': 'tableType',
            'value': 'tableType',
            'enable': true
        }
    ],
    'Overbridge': [
        {
            'label': 'project',
            'value': 'project',
            'enable': false
        },
        {
            'label': 'id',
            'value': 'id',
            'enable': true
        },
        {
            'label': 'section',
            'value': 'sectionID',
            'enable': true
        },
        {
            'label': 'alignmentID',
            'value': 'alignmentID',
            'enable': true
        },
        {
            'label': 'alignmentCnName',
            'value': 'alignmentCnName',
            'enable': true
        },
        {
            'label': 'stationMark',
            'value': 'stationMark',
            'enable': true
        },
        {
            'label': 'station',
            'value': 'station',
            'enable': true
        },
        {
            'label': 'stationChain',
            'value': 'stationChain',
            'enable': true
        },
        {
            'label': 'direction',
            'value': 'direction',
            'enable': true
        },
        {
            'label': 'leftParts',
            'value': 'leftPartsCopy',
            'enable': true
        },
        {
            'label': 'rightParts',
            'value': 'rightPartsCopy',
            'enable': true
        },
        {
            'label': 'desc',
            'value': 'desc',
            'enable': true
        },
        {
            'label': 'bimFile',
            'value': 'bimFiles',
            'enable': true
        },
        {
            'label': 'tableType',
            'value': 'tableType',
            'enable': true
        }
    ],
    'Bridge': [
        {
            'label': 'project',
            'value': 'project',
            'enable': false
        },
        {
            'label': 'id',
            'value': 'id',
            'enable': true
        },
        {
            'label': 'bridgeCnName',
            'value': 'bridgeCnName',
            'enable': true
        },
        {
            'label': 'bridgeEnName',
            'value': 'bridgeEnName',
            'enable': true
        },
        {
            'label': 'section',
            'value': 'sectionID',
            'enable': true
        },
        {
            'label': 'region',
            'value': 'region',
            'enable': true
        },
        {
            'label': 'leftParts',
            'value': 'leftPartsCopy',
            'enable': true
        },
        {
            'label': 'leftAlignmentID',
            'value': 'leftParts.alignmentID',
            'enable': true
        },
        {
            'label': 'leftAlignmentCnName',
            'value': 'leftParts.alignmentCnName',
            'enable': true
        },
        {
            'label': 'leftStationMark',
            'value': 'leftParts.stationMark',
            'enable': true
        },
        {
            'label': 'leftStartStation',
            'value': 'leftParts.startStation',
            'enable': true
        },
        {
            'label': 'leftEndStation',
            'value': 'leftParts.endStation',
            'enable': true
        },
        {
            'label': 'leftStartStationChain',
            'value': 'leftParts.startStationChain',
            'enable': true
        },
        {
            'label': 'leftEndStationChain',
            'value': 'leftParts.endStationChain',
            'enable': true
        },
        {
            'label': 'rightParts',
            'value': 'rightPartsCopy',
            'enable': true
        },
        {
            'label': 'rightAlignmentID',
            'value': 'rightPart.alignmentID',
            'enable': true
        },
        {
            'label': 'rightAlignmentCnName',
            'value': 'rightPart.alignmentCnName',
            'enable': true
        },
        {
            'label': 'rightStationMark',
            'value': 'rightPart.stationMark',
            'enable': true
        },
        {
            'label': 'rightStartStation',
            'value': 'rightPart.startStation',
            'enable': true
        },
        {
            'label': 'rightEndStation',
            'value': 'rightPart.endStation',
            'enable': true
        },
        {
            'label': 'rightStartStationChain',
            'value': 'rightPart.startStationChain',
            'enable': true
        },
        {
            'label': 'rightEndStationChain',
            'value': 'rightPart.endStationChain',
            'enable': true
        },
        {
            'label': 'direction',
            'value': 'direction',
            'enable': true
        },
        {
            'label': 'desc',
            'value': 'desc',
            'enable': true
        },
        {
            'label': 'bimFile',
            'value': 'bimFiles',
            'enable': true
        },
        {
            'label': 'tableType',
            'value': 'tableType',
            'enable': true
        }
    ],
    'Geology': [
        {
            'label': 'project',
            'value': 'project',
            'enable': false
        },
        {
            'label': 'id',
            'value': 'id',
            'enable': true
        },
        {
            'label': 'geologyCnName',
            'value': 'geologyCnName',
            'enable': true
        },
        {
            'label': 'geologyEnName',
            'value': 'geologyEnName',
            'enable': true
        },
        {
            'label': 'section',
            'value': 'sectionID',
            'enable': true
        },
        {
            'label': 'alignmentID',
            'value': 'alignmentID',
            'enable': true
        },
        {
            'label': 'alignmentCnName',
            'value': 'alignmentCnName',
            'enable': true
        },
        {
            'label': 'stationMark',
            'value': 'stationMark',
            'enable': true
        },
        {
            'label': 'startStation',
            'value': 'startStation',
            'enable': true
        },
        {
            'label': 'startStationChain',
            'value': 'startStationChain',
            'enable': true
        },
        {
            'label': 'endStation',
            'value': 'endStation',
            'enable': true
        },
        {
            'label': 'endStationChain',
            'value': 'endStationChain',
            'enable': true
        },
        {
            'label': 'leftParts',
            'value': 'leftPartsCopy',
            'enable': true
        },
        {
            'label': 'rightParts',
            'value': 'rightPartsCopy',
            'enable': true
        },
        {
            'label': 'desc',
            'value': 'desc',
            'enable': true
        },
        {
            'label': 'bimFile',
            'value': 'bimFiles',
            'enable': true
        },
        {
            'label': 'tableType',
            'value': 'tableType',
            'enable': true
        }
    ],
    'Tunnel': [
        {
            'label': 'project',
            'value': 'project',
            'enable': false
        },
        {
            'label': 'id',
            'value': 'id',
            'enable': true
        },
        {
            'label': 'tunnelEnName',
            'value': 'tunnelEnName',
            'enable': true
        },
        {
            'label': 'tunnelCnName',
            'value': 'tunnelCnName',
            'enable': true
        },
        {
            'label': 'section',
            'value': 'sectionID',
            'enable': true
        },
        {
            'label': 'leftParts',
            'value': 'leftPartsCopy',
            'enable': true
        },
        {
            'label': 'leftAlignmentID',
            'value': 'leftParts.alignmentID',
            'enable': true
        },
        {
            'label': 'leftAlignmentCnName',
            'value': 'leftParts.alignmentCnName',
            'enable': true
        },
        {
            'label': 'leftStationMark',
            'value': 'leftParts.stationMark',
            'enable': true
        },
        {
            'label': 'leftStartStation',
            'value': 'leftParts.startStation',
            'enable': true
        },
        {
            'label': 'leftEndStation',
            'value': 'leftParts.endStation',
            'enable': true
        },
        {
            'label': 'leftStartStationChain',
            'value': 'leftParts.startStationChain',
            'enable': true
        },
        {
            'label': 'leftEndStationChain',
            'value': 'leftParts.endStationChain',
            'enable': true
        },
        {
            'label': 'rightParts',
            'value': 'rightPartsCopy',
            'enable': true
        },
        {
            'label': 'rightAlignmentID',
            'value': 'rightPart.alignmentID',
            'enable': true
        },
        {
            'label': 'rightAlignmentCnName',
            'value': 'rightPart.alignmentCnName',
            'enable': true
        },
        {
            'label': 'rightStationMark',
            'value': 'rightPart.stationMark',
            'enable': true
        },
        {
            'label': 'rightStartStation',
            'value': 'rightPart.startStation',
            'enable': true
        },
        {
            'label': 'rightEndStation',
            'value': 'rightPart.endStation',
            'enable': true
        },
        {
            'label': 'rightStartStationChain',
            'value': 'rightPart.startStationChain',
            'enable': true
        },
        {
            'label': 'rightEndStationChain',
            'value': 'rightPart.endStationChain',
            'enable': true
        },
        {
            'label': 'direction',
            'value': 'direction',
            'enable': true
        },
        {
            'label': 'desc',
            'value': 'desc',
            'enable': true
        },
        {
            'label': 'bimFile',
            'value': 'bimFiles',
            'enable': true
        },
        {
            'label': 'tableType',
            'value': 'tableType',
            'enable': true
        }
    ],
    'Softbase': [
        {
            'label': 'project',
            'value': 'project',
            'enable': false
        },
        {
            'label': 'id',
            'value': 'id',
            'enable': true
        },
        {
            'label': 'type',
            'value': 'type',
            'enable': true
        },
        {
            'label': 'modelType',
            'value': 'modelType',
            'enable': true
        },
        {
            'label': 'section',
            'value': 'sectionID',
            'enable': true
        },
        {
            'label': 'alignmentID',
            'value': 'alignmentID',
            'enable': true
        },
        {
            'label': 'alignmentCnName',
            'value': 'alignmentCnName',
            'enable': true
        },
        {
            'label': 'stationMark',
            'value': 'stationMark',
            'enable': true
        },
        {
            'label': 'startStation',
            'value': 'startStation',
            'enable': true
        },
        {
            'label': 'endStation',
            'value': 'endStation',
            'enable': true
        },
        {
            'label': 'startStationChain',
            'value': 'startStationChain',
            'enable': true
        },
        {
            'label': 'endStationChain',
            'value': 'endStationChain',
            'enable': true
        },
        {
            'label': 'leftParts',
            'value': 'leftPartsCopy',
            'enable': true
        },
        {
            'label': 'rightParts',
            'value': 'rightPartsCopy',
            'enable': true
        },
        {
            'label': 'desc',
            'value': 'desc',
            'enable': true
        },
        {
            'label': 'tableType',
            'value': 'tableType',
            'enable': true
        }
    ],
    'Guardwall': [
        {
            'label': 'project',
            'value': 'project',
            'enable': false
        },
        {
            'label': 'id',
            'value': 'id',
            'enable': true
        },
        {
            'label': 'type',
            'value': 'type',
            'enable': true
        },
        {
            'label': 'modelType',
            'value': 'modelType',
            'enable': true
        },
        {
            'label': 'section',
            'value': 'sectionID',
            'enable': true
        },
        {
            'label': 'alignmentID',
            'value': 'alignmentID',
            'enable': true
        },
        {
            'label': 'alignmentCnName',
            'value': 'alignmentCnName',
            'enable': true
        },
        {
            'label': 'stationMark',
            'value': 'stationMark',
            'enable': true
        },
        {
            'label': 'startStation',
            'value': 'startStation',
            'enable': true
        },
        {
            'label': 'endStation',
            'value': 'endStation',
            'enable': true
        },
        {
            'label': 'startStationChain',
            'value': 'startStationChain',
            'enable': true
        },
        {
            'label': 'endStationChain',
            'value': 'endStationChain',
            'enable': true
        },
        {
            'label': 'leftParts',
            'value': 'leftPartsCopy',
            'enable': true
        },
        {
            'label': 'rightParts',
            'value': 'rightPartsCopy',
            'enable': true
        },
        {
            'label': 'desc',
            'value': 'desc',
            'enable': true
        },
        {
            'label': 'direction',
            'value': 'direction',
            'enable': true
        },
        {
            'label': 'tableType',
            'value': 'tableType',
            'enable': true
        }
    ],
    'SideBlinddrain': [
        {
            'label': 'project',
            'value': 'project',
            'enable': false
        },
        {
            'label': 'id',
            'value': 'id',
            'enable': true
        },
        {
            'label': 'type',
            'value': 'type',
            'enable': true
        },
        {
            'label': 'modelType',
            'value': 'modelType',
            'enable': true
        },
        {
            'label': 'section',
            'value': 'sectionID',
            'enable': true
        },
        {
            'label': 'alignmentID',
            'value': 'alignmentID',
            'enable': true
        },
        {
            'label': 'alignmentCnName',
            'value': 'alignmentCnName',
            'enable': true
        },
        {
            'label': 'stationMark',
            'value': 'stationMark',
            'enable': true
        },
        {
            'label': 'startStation',
            'value': 'startStation',
            'enable': true
        },
        {
            'label': 'endStation',
            'value': 'endStation',
            'enable': true
        },
        {
            'label': 'startStationChain',
            'value': 'startStationChain',
            'enable': true
        },
        {
            'label': 'endStationChain',
            'value': 'endStationChain',
            'enable': true
        },
        {
            'label': 'leftParts',
            'value': 'leftPartsCopy',
            'enable': true
        },
        {
            'label': 'rightParts',
            'value': 'rightPartsCopy',
            'enable': true
        },
        {
            'label': 'desc',
            'value': 'desc',
            'enable': true
        },
        {
            'label': 'direction',
            'value': 'direction',
            'enable': true
        },
        {
            'label': 'tableType',
            'value': 'tableType',
            'enable': true
        }
    ],
    'CrossBlinddrain': [
        {
            'label': 'project',
            'value': 'project',
            'enable': false
        },
        {
            'label': 'id',
            'value': 'id',
            'enable': true
        },
        {
            'label': 'type',
            'value': 'type',
            'enable': true
        },
        {
            'label': 'modelType',
            'value': 'modelType',
            'enable': true
        },
        {
            'label': 'section',
            'value': 'sectionID',
            'enable': true
        },
        {
            'label': 'alignmentID',
            'value': 'alignmentID',
            'enable': true
        },
        {
            'label': 'alignmentCnName',
            'value': 'alignmentCnName',
            'enable': true
        },
        {
            'label': 'stationMark',
            'value': 'stationMark',
            'enable': true
        },
        {
            'label': 'startStation',
            'value': 'startStation',
            'enable': true
        },
        {
            'label': 'endStation',
            'value': 'endStation',
            'enable': true
        },
        {
            'label': 'startStationChain',
            'value': 'startStationChain',
            'enable': true
        },
        {
            'label': 'endStationChain',
            'value': 'endStationChain',
            'enable': true
        },
        {
            'label': 'leftParts',
            'value': 'leftPartsCopy',
            'enable': true
        },
        {
            'label': 'rightParts',
            'value': 'rightPartsCopy',
            'enable': true
        },
        {
            'label': 'desc',
            'value': 'desc',
            'enable': true
        },
        {
            'label': 'direction',
            'value': 'direction',
            'enable': true
        },
        {
            'label': 'tableType',
            'value': 'tableType',
            'enable': true
        }
    ],
    //
    Bridge_Model: [
        {
            'label': 'project',
            'value': 'project',
            'enable': false
        },
        {
            'label': 'idx',
            'value': 'id',
            'enable': true
        },
        {
            'label': 'bridgeCnName',
            'value': 'bridgeCnName',
            'enable': true
        },
        {
            'label': 'bridgeEnName',
            'value': 'bridgeEnName',
            'enable': true
        },
        {
            'label': 'section',
            'value': 'sectionID',
            'enable': true
        },
        {
            'label': 'region',
            'value': 'region',
            'enable': true
        },
        {
            'label': 'rightAlignmentID',
            'value': 'rightPart.alignmentID',
            'enable': true
        },
        {
            'label': 'rightAlignmentCnName',
            'value': 'rightPart.alignmentCnName',
            'enable': true
        },
        {
            'label': 'x',
            'value': 'posture.x',
            'enable': true
        },
        {
            'label': 'y',
            'value': 'posture.y',
            'enable': true
        },
        {
            'label': 'z',
            'value': 'posture.z',
            'enable': true
        },
        {
            'label': 'angle',
            'value': 'posture.alpha',
            'enable': true
        },
        {
            'label': 'leftParts',
            'value': 'leftPartsCopy',
            'enable': true
        },
        {
            'label': 'rightParts',
            'value': 'rightPartsCopy',
            'enable': true
        },
        {
            'label': 'desc',
            'value': 'desc',
            'enable': true
        },
        {
            'label': 'bimFile',
            'value': 'bimFiles',
            'enable': true
        }
    ],
    Overbridge_Model: [
        {
            'label': 'project',
            'value': 'project',
            'enable': false
        },
        {
            'label': 'idx',
            'value': 'id',
            'enable': true
        },
        {
            'label': 'section',
            'value': 'sectionID',
            'enable': true
        },
        {
            'label': 'alignmentID',
            'value': 'alignmentID',
            'enable': true
        },
        {
            'label': 'alignmentCnName',
            'value': 'alignmentCnName',
            'enable': true
        },
        {
            'label': 'stationMark',
            'value': 'stationMark',
            'enable': true
        },
        {
            'label': 'station',
            'value': 'station',
            'enable': true
        },
        {
            'label': 'stationChain',
            'value': 'stationChain',
            'enable': true
        },
        {
            'label': 'x',
            'value': 'posture.x',
            'enable': true
        },
        {
            'label': 'y',
            'value': 'posture.y',
            'enable': true
        },
        {
            'label': 'z',
            'value': 'posture.z',
            'enable': true
        },
        {
            'label': 'angle',
            'value': 'posture.alpha',
            'enable': true
        },
        {
            'label': 'direction',
            'value': 'direction',
            'enable': true
        },
        {
            'label': 'leftParts',
            'value': 'leftPartsCopy',
            'enable': true
        },
        {
            'label': 'rightParts',
            'value': 'rightPartsCopy',
            'enable': true
        },
        {
            'label': 'desc',
            'value': 'desc',
            'enable': true
        },
        {
            'label': 'bimFile',
            'value': 'bimFiles',
            'enable': true
        },
        {
            'label': 'tableType',
            'value': 'tableType',
            'enable': true
        }
    ],
    Culvert_Model: [
        {
            'label': 'project',
            'value': 'project',
            'enable': false
        },
        {
            'label': 'id',
            'value': 'id',
            'enable': true
        },
        {
            'label': 'section',
            'value': 'sectionID',
            'enable': true
        },
        {
            'label': 'alignmentID',
            'value': 'alignmentID',
            'enable': true
        },
        {
            'label': 'alignmentCnName',
            'value': 'alignmentCnName',
            'enable': true
        },
        {
            'label': 'stationMark',
            'value': 'stationMark',
            'enable': true
        },
        {
            'label': 'station',
            'value': 'station',
            'enable': true
        },
        {
            'label': 'stationChain',
            'value': 'stationChain',
            'enable': true
        },
        {
            'label': 'x',
            'value': 'posture.x',
            'enable': true
        },
        {
            'label': 'y',
            'value': 'posture.y',
            'enable': true
        },
        {
            'label': 'z',
            'value': 'posture.z',
            'enable': true
        },
        {
            'label': 'angle',
            'value': 'posture.alpha',
            'enable': true
        },
        {
            'label': 'leftParts',
            'value': 'leftPartsCopy',
            'enable': true
        },
        {
            'label': 'rightParts',
            'value': 'rightPartsCopy',
            'enable': true
        },
        {
            'label': 'desc',
            'value': 'desc',
            'enable': true
        },
        {
            'label': 'bimFile',
            'value': 'bimFiles',
            'enable': true
        },
        {
            'label': 'angle',
            'value': 'angle',
            'enable': true
        },
        {
            'label': 'direction',
            'value': 'direction',
            'enable': true
        },
        {
            'label': 'tableType',
            'value': 'tableType',
            'enable': true
        }
    ],
};

export default NodeField;
